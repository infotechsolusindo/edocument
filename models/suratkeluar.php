...
class SuratKeluar extends Dokumen {
    ...
    public function getDiterima() {
        $sql = "select * from dokumen where tipe = '2' and status = '2' order by tgl desc";
        return $this->_db->Exec($sql);
    }
    public function getKembali() {
        $sql = "select * from dokumen where tipe = '2' and status = '3' order by tgl desc";
        return $this->_db->Exec($sql);
    }
    public function getDitolak() {
        $sql = "select * from dokumen where tipe = '2' and status = 'x' order by tgl desc";
        return $this->_db->Exec($sql);
    }
    public function getDihapus() {
        $sql = "select * from dokumen where tipe = '2' and status = 'D' order by tgl desc";
        return $this->_db->Exec($sql);
    }
